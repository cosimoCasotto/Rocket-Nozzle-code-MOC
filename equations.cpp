...
float equations::getExitMach(float Tc, float Pc, float Pe, float &gam, float R){
	...
	float 
equations::getPMAngle(float gam, float M)
{
	if (gam <= 1){
		return 0;
	}
	...
	float 
equations::getMachFromPM(float Mi, float pmAngle, float gam)
{
	...
	std::vector<float> 
equations::interpolateAngle(float theta, int n)
{
	...
	std::string 
equations::vectorToString(std::vector<float> v)
{
	std::string str;
	...
	std::string 
equations::vectorToString(std::vector<int> v)
{
	std::string str;
	for (unsigned int i = 0; i < v.size(); i++){
		str.append(std::to_string(v[i]));
...
equations::returnXYIntersectionPoint(float xt, float yt, float tht_to, float xb, float yb, float tht_b)
{
